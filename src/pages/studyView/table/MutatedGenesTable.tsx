...
export class MutatedGenesTable extends React.Component<IMutatedGenesTablePros, {}> {
    ...
    get tableColumns() {
        return [{
            ...
            render: (data: MutationCountByGene) => {
                const genePanelString = data.matchingGenePanels.join(", ")
                const addTotalProfiledOverlay = () =>
                    <span>{`# of samples profiled for mutations in this gene: ${data.numberOfSamplesProfiled}, gene panels: ${genePanelString}`}</span>
                return (
                    <DefaultTooltip
                        placement="right"
