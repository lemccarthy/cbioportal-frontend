...
export function buildDefaultOQLProfile(
    ...
    for (let i = 0; i < profilesTypes.length; i++) {
        const type = profilesTypes[i];
        switch (type) {
...
export class ResultsViewPageStore {
    ...
    @observable private _showGermlineMutations = true;
    ...
    @observable expressionTabSeqVersion = 2;
    ...
    public initDriverAnnotationSettings() {
        ...
        (this.driverAnnotationSettings as any)._oncoKb = !AppConfig.serverConfig
            .oncoprint_oncokb_hotspots_default;
        (this.driverAnnotationSettings as any)._ignoreUnknown = !!AppConfig.serverConfig
    ...
    public groupSamplesByCancerType(
        ...
        const samplesGroupedByCancerType = _.reduce(
            samples,
    ...
    readonly oqlFilteredAlterationsByGeneBySampleKey = remoteData<{
        [hugoGeneSymbol: string]: { [uniquSampleKey: string]: ExtendedAlteration[] };
    }>({
    ...
    readonly mutations = remoteData<Mutation[]>({
        ...
        invoke: async () => {
            ...
            const studyIdToProfileMap: { [studyId: string]: MolecularProfile } = _.keyBy(
                mutationProfiles,
                ...
                if (this.genes.result) {
                    ...
                    return Promise.resolve(
                        _.reduce(
                            this.genes.result,
                            (
                                map: { [hugoGeneSymbol: string]: ResultsViewMutationMapperStore },
                                gene: Gene
                            ) => {
    ...
    readonly survivalClinicalData = remoteData<ClinicalData[]>(
        {
            await: () => [this.studies, this.patients],
            invoke: () =>
                this.getClinicalData("PATIENT", this.studies.result!, this.patients.result, [
                    "OS_STATUS",
                    "OS_MONTHS",
                    "DFS_STATUS",
                    "DFS_MONTHS"
    ...
    readonly patients = remoteData({
        await: () => [this.samples],
        invoke: () => {
            const patientKeyToPatientIdentifier: {
            ...
            const patientFilter = {
                uniquePatientKeys: _.uniq(
                    this.samples.result.map((sample: Sample) => sample.uniquePatientKey)
                )
            } as PatientFilter;
        ...
        invoke: () => {
            return Promise.resolve(
                this.molecularProfilesInStudies.result.filter((profile: MolecularProfile) =>
                    isRNASeqProfile(profile.molecularProfileId, this.expressionTabSeqVersion)
                )
            );
        }
    });
    ...
    readonly rnaSeqMolecularData = remoteData<{
        ...
        await: () => [this.expressionProfiles, this.genes, this.geneMolecularDataCache],
        invoke: async () => {
            ...
            const keyedByHugoSymbol = _.mapKeys(groupedByGene, (val, entrezGeneId: string) => {
                ...
                return _.find(
                    this.genes.result,
                    (gene: Gene) => gene.entrezGeneId.toString() === entrezGeneId
                )!.hugoGeneSymbol;
            });
            ...
            const unwrapCacheObjects: {
                ...
                keyedByHugoSymbol,
                (val: AugmentedData<NumericGeneMolecularData[], GeneMolecularDataCache>) => {
                    return _.map(
                        val,
                        (item: AugmentedData<NumericGeneMolecularData[], GeneMolecularDataCache>) =>
                            item.data
                    );
                }
