...
export function scatterPlotLegendData(
  ...
  if (viewType === ViewType.CopyNumber && _cnaDataExists) {
    ...
    return scatterPlotMutationLegendData(data, driversAnnotated, false).concat(
      scatterPlotCnaLegendData(data)
    );
  }
  return [];
}
  ...
  switch (selection.dataType) {
    ...
    default:
      ...
      if (profile && selection.entrezGeneId !== undefined) {
        ret = `${entrezGeneIdToGene[selection.entrezGeneId].hugoGeneSymbol}: ${
          profile.name
        }`;
      }
      break;
...
function generalScatterPlotTooltip<D extends IScatterPlotSampleData>(
  ...
  return (
    <div>
      ...
      <div>
        Horizontal:{" "}
        <span style={{ fontWeight: "bold" }}>{d[horizontalKey] as any}</span>
      </div>
      <div>
        Vertical:{" "}
        <span style={{ fontWeight: "bold" }}>{d[verticalKey] as any}</span>
      </div>
      {mutationsSection}
      {!!mutationsSection && <br />}
      {cnaSection}
