  ...
  private initAxisMenuSelection(vertical: boolean): AxisMenuSelection {
    ...
    return observable({
      ...
      get selectedGeneOption() {
        const geneOptions =
          (vertical ? self.vertGeneOptions : self.horzGeneOptions.result) || [];
      ...
      get selectedGenesetOption() {
        ...
        if (
          ...
          this._selectedGenesetOption.value === SAME_GENESET_OPTION_VALUE &&
          self.horzSelection.dataType === CLIN_ATTR_DATA_TYPE
        ) {
      ...
      return Promise.resolve(
        _.flatten(
          this.props.store.putativeDriverAnnotatedMutationCache
            .getAll(getMutationQueries(this.horzSelection, this.vertSelection))
            .map(p => p.result!)
        ).filter(x => !!x)
      );
    }
    ...
    return (d: IScatterPlotSampleData) => {
      ...
      for (const word of searchMutationWords) {
        mutationMatch =
          ...
          (!!word.length &&
            !!d.mutations.find(
              m => !!(m.proteinChange && m.proteinChange.indexOf(word) > -1)
            ));
        if (mutationMatch) {
          break;
        }
        ...
        <div>
          ...
          {axisSelection.dataType !== GENESET_DATA_TYPE && (
            <div
              ...
              style={{
                opacity: axisSelection.dataType === CLIN_ATTR_DATA_TYPE ? 0 : 1
              }}
            >
              <label>Gene</label>
              ...
              plotElt = (
                <PlotsTabBoxPlot
                  ...
                  axisLabelX={
                    ...
                    (horizontal ? this.horzLabelLogSuffix : "")
                  }
                  axisLabelY={
                    this.vertLabel.result! +
                    (!horizontal ? this.vertLabelLogSuffix : "")
                  }
