  ...
  private coExpressionCache: CoExpressionCache = new CoExpressionCache(
    q => ({
      invoke: () => {
        ...
        const dataQueryFilter = this.props.store.studyToDataQueryFilter.result![
          q.molecularProfile.studyId
        ];
        if (dataQueryFilter) {
  ...
  private geneTabs() {
    if (
      ...
      for (const gene of this.props.store.genes.result!) {
        for (const profile of this.props.store.coexpressionTabMolecularProfiles
          .result) {
