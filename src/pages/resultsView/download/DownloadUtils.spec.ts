...
describe("DownloadUtils", () => {
  ...
  describe("generateCaseAlterationData", () => {
    it("properly handles not sequenced genes when generating case alteration data", () => {
      ...
      assert.isFalse(
        caseAlterationData[0].oqlData["EGFR: AMP HOMDEL MUT FUSION;"].sequenced,
        "cases with the gene EGFR should be marked as not sequenced"
      );

      assert.isTrue(
        caseAlterationData[1].oqlData["PTEN: AMP HOMDEL MUT FUSION;"].sequenced,
    ...
    it("generates case alteration data for multiple samples", () => {
      ...
      assert.deepEqual(
        caseAlterationData[0].oqlData["EGFR: AMP HOMDEL MUT FUSION;"].mutation,
        ["G598A", "G239C"],
        "mutation data is correct for the sample key UC0wMDAwMzc4LVQwMS1JTTM6bXNrX2ltcGFjdF8yMDE3"
      );
      assert.deepEqual(
        caseAlterationData[0].oqlData["EGFR: AMP HOMDEL MUT FUSION;"].fusion,
        ["EGFR-intragenic"],
      ...
      assert.deepEqual(
        caseAlterationData[1].oqlData["PTEN: AMP HOMDEL MUT FUSION;"].mrnaExp,
        [{ type: "HIGH", value: 2.4745 }],
